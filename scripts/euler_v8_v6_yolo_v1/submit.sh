#!/usr/bin/env bash
# spatial_v6_3m single-arm launcher (smoke on 1x3090, screen on 1x4090).
#
#   SUBMIT=0 scripts/euler_v8_v6_yolo_v1/submit.sh smoke  <runtime-terra-sha>
#   SUBMIT=1 scripts/euler_v8_v6_yolo_v1/submit.sh smoke  <runtime-terra-sha>
#   SUBMIT=1 scripts/euler_v8_v6_yolo_v1/submit.sh screen <runtime-terra-sha>
#
# The runtime Terra revision is an argument (currently
# eb3835c1d17af81e970b973ed5abf687ca6f3a26); the bank protocol revision stays
# frozen at a6e6e5bc inside run.sbatch.
set -euo pipefail
...
SEED=20260807
ARM=v6_3m
CAMPAIGN_ID=terra_v8_v6_yolo_v1
...
PAIR_ID="v8_v6_yolo_v1_${BASELINES_REVISION:0:12}_${RUNTIME_TERRA_REVISION:0:12}_s${SEED}"

echo "phase=$PHASE arm=$ARM support=all47_continuous_v2 reward=dense_skill seed=$SEED updates=$([ "$PHASE" = smoke ] && echo 1 || echo 20000)"
...
RUN_DIR="$REMOTE_RUNS/$BASELINES_REVISION/$PHASE/all47/s$SEED/$ARM"
ssh "$REMOTE_HOST" "test ! -e '$RUN_DIR'"
...
EXPORTS="ALL,PHASE=$PHASE,BASELINES_ROOT=$REMOTE_SOURCE,BASELINES_REVISION=$BASELINES_REVISION,SEED=$SEED,RUNTIME_TERRA_ROOT=$REMOTE_TERRA,RUNTIME_TERRA_REVISION=$RUNTIME_TERRA_REVISION,BANK_ARCHIVE=$REMOTE_BANK,BANK_SHA=$BANK_SHA,BANK_DATASET_SHA=$BANK_DATASET_SHA,BANK_RELEASE_ID=$RELEASE_ID,SMOKE_JOB_ID=$SMOKE_JOB_ID,SMOKE_RUN=$SMOKE_RUN"
JOB_ID="$(ssh "$REMOTE_HOST" "cat '$REMOTE_SOURCE/scripts/euler_v8_v6_yolo_v1/run.sbatch' | sbatch --parsable --partition='$PARTITION' --time='$WALLTIME' --gpus='$GPU_TYPE:1' --exclude='eu-g6-064' --job-name='terra-v8-${ARM}' --output='$RUN_DIR/slurm_%j.out' --export='$EXPORTS'")"
