...
if [ "$#" -ne 4 ]; then
    echo "usage: run_v8_scratch_capability.sh BANK_ROOT RUN_NAME NUM_UPDATES RUN_ROOT" >&2
    exit 2
fi
...
exec "$PYTHON_BIN" -u "$REPO/train_mixed.py" \
    ...
    --accepted-bank-stage capability \
    --accepted-bank-sampler-profile bank_v4 \
    ...
    --ent_schedule_steps "$NUM_UPDATES" \
    --no_value_clip \
