...
if [ "$#" -ne 7 ]; then
    echo "usage: run_trench_align_generalist_partial_v1.sh BANK_ROOT PARTIAL_ROOT RUN_NAME TARGET_UPDATES RUN_ROOT DISTANCE_SHA RESUME_OR_NONE" >&2
    exit 2
fi
...
: "${PROTOCOL_TERRA_REVISION:?set the accepted-bank Terra revision}"
...
exec "$PYTHON_BIN" -u "$REPO/train_mixed.py" \
    --config trench_align_generalist_partial_v1 \
    ...
    --accepted-bank-condition-profile trench_aligned_37_v1 \
