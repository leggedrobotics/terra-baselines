...
if [ "$#" -ne 5 ]; then
    echo "usage: run_v8_architecture_control_v1.sh compact_xattn|atari_base BANK_ROOT RUN_NAME UPDATES RUN_ROOT" >&2
    exit 2
fi
...
case "$ARM" in compact_xattn|atari_base) ;; *) echo "invalid arm '$ARM'" >&2; exit 2 ;; esac
