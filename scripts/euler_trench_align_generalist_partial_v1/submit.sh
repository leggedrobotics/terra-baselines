...
printf '%s\n' \
    ...
    "xla_gpu_autotune_level=4 encoder_compute_dtype=float32 excluded_nodes=$EXCLUDED_NODES" \
...
if [ "$SUBMIT" = recovery ]; then
    ...
    [[ "$TAIL_JOB_ID" =~ ^[0-9]+$ ]] || {
        ...
        remote "printf '%s\n' 'status=SUBMITTED' 'source_checkpoint=$RECOVERY_CHECKPOINT' 'source_checkpoint_sha256=$RECOVERY_CHECKPOINT_SHA' 'source_update=$RECOVERY_UPDATE' 'source_wandb_run_id=$PARENT_WANDB_RUN_ID' 'throughput_smoke_1gpu_job_id=$ONE_GPU_JOB_ID' 'throughput_smoke_4gpu_job_id=$SMOKE_JOB_ID' 'recovery_job_id=$RECOVERY_JOB_ID' 'tail_job_id=$TAIL_JOB_ID' 'dependencies=throughput_4gpu:afterok:$ONE_GPU_JOB_ID,recovery:afterok:$SMOKE_JOB_ID,tail:afterok:$RECOVERY_JOB_ID' 'terra_baselines_revision=$BASELINES_REVISION' 'runtime_terra_revision=$RUNTIME_TERRA_REVISION' 'partial_reset_bank_sha256=$PARTIAL_BANK_SHA' 'xla_gpu_autotune_level=4' 'encoder_compute_dtype=float32' 'attention_compute_dtype=float32' 'excluded_nodes=$EXCLUDED_NODES' > '$RECOVERY_PARENT/submission.env'"
    printf '%s\n' \
