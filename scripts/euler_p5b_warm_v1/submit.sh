...
TERRA_REPO="/home/lorenzo/moleworks/.worktrees/terra_simple_mapbank_reward_20260730"
...
REMOTE_WORK="/cluster/home/lterenzi/codex_terra_edge_validation/p5b_warm_v1"
REMOTE_INPUTS="/cluster/scratch/lterenzi/codex_terra_edge_runs/p5b_warm_v1/inputs"
REMOTE_RUNS="/cluster/scratch/lterenzi/codex_terra_edge_runs/p5b_warm_v1"
ARMS=(G-MEDIUM-ADAPTIVE-WARM G-DEEP-ADAPTIVE-WARM G-MEDIUM-UNIFORM-WARM)
...
case "$PHASE" in
    ...
    screen) PARTITION=gpuhe.24h; WALLTIME=12:00:00 ;;
esac
for ARM in "${ARMS[@]}"; do
    ...
    EXPORTS="ALL,PHASE=$PHASE,ARM=$ARM,BASELINES_ROOT=$REMOTE_SOURCE,BASELINES_REVISION=$BASELINES_REVISION,DEEP_CHECKPOINT=$REMOTE_DEEP,DEEP_SHA=$DEEP_SHA,SEED=$SEED"
    JOB_ID="$(
        ssh "$REMOTE_HOST" "
            cat '$REMOTE_SOURCE/scripts/euler_p5b_warm_v1/run.sbatch' |
            sbatch --parsable --partition='$PARTITION' --time='$WALLTIME' \\
                --exclude='eu-g6-064' --job-name='terra-p5b-${PHASE}-${ARM}' \\
