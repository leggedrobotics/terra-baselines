...
for index in "${!LABELS[@]}"; do
    ...
    exports="ALL,POLICY_LABEL=$label,CHECKPOINT_PATH=$checkpoint,CHECKPOINT_SHA256=$checkpoint_sha,BASELINES_ROOT=$BASELINES_ROOT,BASELINES_REVISION=$BASELINES_REVISION,TERRA_ROOT=$TERRA_ROOT,TERRA_REVISION=$TERRA_REVISION,EPISODE_BANK_ROOT=$EPISODE_BANK_ROOT,EPISODE_BANK_JSON_SHA256=$EPISODE_BANK_JSON_SHA256,EPISODE_BANK_FILES_SHA256=$EPISODE_BANK_FILES_SHA256,PYTHON_BIN=$PYTHON_BIN,OUTPUT_ROOT=$output_root"
    command=(
        sbatch --parsable
        --job-name="$job_name"
        --chdir="$OUTPUT_BASE"
