...
Use `lterenzi` through SSH alias `euler-lterenzi`. The former Weber account is
closed; remove its SSH alias and dedicated credentials from active clients.
...
`lterenzi` may read a historical shared dataset in
`/cluster/project/rsl/alesweber/TerraProject/terra/data/terra/train` and a
group-readable pinned venv owned by another RSL account. Verify current
readability, record both paths in the run contract, and validate the runtime
inside every GPU allocation. W&B entity `aless-weber-eth` is an experiment
identity and does not select the Unix login.
...
Use the active account alias and an account-specific multiplexing socket:
...
```bash
ssh -o BatchMode=yes euler-lterenzi 'id -un; printf "%s\n" "$HOME"; lquota'
```
...
Legacy scripts directly under `cluster/` require explicit `CONDA_ROOT`,
`TERRA_BASELINES_ROOT` (or `TERRA_ROOT` for the manual Terra viewer), and
`DATASET_PATH`; evaluation/inference scripts also require `CHECKPOINT`.
They do not infer a runtime, checkout, or checkpoint from the former account.
...
```bash
ssh euler-lterenzi 'squeue -u "$USER" -o "%.18i %.9T %.12M %.30j %.20N %.24R"'
ssh euler-lterenzi 'sacct -j JOBID --format=JobID,State,ExitCode,Elapsed,AllocTRES -P'
```
