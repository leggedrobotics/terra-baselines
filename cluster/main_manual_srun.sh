...
: "${TERRA_ROOT:?Set TERRA_ROOT to the staged Terra checkout}"
: "${DATASET_PATH:?Set DATASET_PATH to the selected readable map bank}"

# Recommended srun command (if running from login node):
# srun --x11 --cpus-per-task=1 --mem-per-cpu=4G terra-baselines/cluster/main_manual_srun.sh
...
# Option 1: X11 forwarding (connect with: ssh -Y euler-lterenzi)
