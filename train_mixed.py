...
def _flatten_param_shapes(tree, prefix=""):
    """Collect parameter leaf shapes with readable Flax path names."""
    if hasattr(tree, "items"):
        out = []
        for key, value in tree.items():
            key = str(key)
            child_prefix = f"{prefix}/{key}" if prefix else key
            out.extend(_flatten_param_shapes(value, child_prefix))
        return out
    if hasattr(tree, "shape"):
        return [(prefix, tuple(tree.shape))]
    return []


def _print_macro_policy_debug(config, env, network_params):
    """One-time startup debug to verify we are training the macro action model."""
    map_size = env.batch_cfg.maps_dims.maps_edge_length
    num_actions = env.batch_cfg.action_type.get_num_actions()
    action_cls_name = getattr(env.batch_cfg.action_type, "__name__", str(env.batch_cfg.action_type))
    print("\n🔎 Macro movement policy debug", flush=True)
    print(f"   terra package path: {getattr(terra, '__file__', '<namespace package>')}", flush=True)
    print(f"   action class: {action_cls_name}", flush=True)
    print(f"   num_actions: {num_actions}", flush=True)
    print(
        "   macro ids: "
        f"MOVE_TO_POSE={int(TrackedActionType.MOVE_TO_POSE)}, "
        f"SET_CABIN_ANGLE={int(TrackedActionType.SET_CABIN_ANGLE)}, "
        f"SET_BASE_ANGLE={int(TrackedActionType.SET_BASE_ANGLE)}",
        flush=True,
    )
    print(
        "   wheeled macro ids: "
        f"MOVE_TO_POSE={int(WheeledActionType.MOVE_TO_POSE)}, "
        f"SET_CABIN_ANGLE={int(WheeledActionType.SET_CABIN_ANGLE)}, "
        f"SET_BASE_ANGLE={int(WheeledActionType.SET_BASE_ANGLE)}",
        flush=True,
    )
    print("   policy action pack: [action_type, move_xy, base_angle, cabin_angle]", flush=True)
    print(
        "   expected head sizes: "
        f"action={num_actions}, move_xy={map_size * map_size}, "
        f"base_angle={env.batch_cfg.agent.angles_base}, "
        f"cabin_angle={env.batch_cfg.agent.angles_cabin}",
        flush=True,
    )
    print(
        f"   config model_size={getattr(config, 'model_size', 'base')}, "
        f"num_prev_actions={config.num_prev_actions}",
        flush=True,
    )

    shape_by_path = _flatten_param_shapes(network_params)
    for head_name in ("mlp_pi", "mlp_move_xy", "mlp_move_angle", "mlp_cabin_angle"):
        matches = [
            (path, shape)
            for path, shape in shape_by_path
            if head_name in path and (path.endswith("kernel") or path.endswith("bias"))
        ]
        print(f"   {head_name} params:", flush=True)
...
def _foundation_border_mask_from_target(target_map, border_width: int):
    dig_target = target_map < 0
    border_width = max(int(border_width), 0)
    ...
    def _erode_once(mask):
        ...
        return jnp.logical_and(mask, neigh_sum == 9.0)
...
def digging_progress_metrics(timestep):
    target_map = timestep.state.world.target_map.map
    action_map = timestep.state.world.action_map.map
    border_width = safe_jax_to_python(timestep.env_cfg.foundation_border_width_tiles[0])
    edge_mask = _foundation_border_mask_from_target(target_map, border_width)
    dig_mask = target_map < 0
    inner_mask = jnp.logical_and(dig_mask, jnp.logical_not(edge_mask))

    def _completion(region_mask):
        required = jnp.where(region_mask, -target_map, 0.0)
        completed = jnp.where(
            region_mask,
            jnp.clip(-action_map, a_min=0.0, a_max=required),
            0.0,
        )
        required_sum = jnp.sum(required, axis=(-2, -1))
        completed_sum = jnp.sum(completed, axis=(-2, -1))
        return jnp.where(
            required_sum > 0,
            completed_sum / jnp.maximum(required_sum, 1e-6),
            jnp.ones_like(required_sum),
        )

    edge_completion = _completion(edge_mask)
    inner_completion = _completion(inner_mask)
    total_completion = _completion(dig_mask)
    return {
        "progress/dig_completion_edge": safe_jax_to_python(jnp.mean(edge_completion)),
        "progress/dig_completion_inner": safe_jax_to_python(jnp.mean(inner_completion)),
        "progress/dig_completion_min_edge_inner": safe_jax_to_python(
...
def episode_progress_metrics(timestep):
    max_steps = jnp.maximum(timestep.env_cfg.max_steps_in_episode, 1)
    task_done = timestep.info.get("task_done", jnp.zeros_like(timestep.done))
    timeout = jnp.logical_and(timestep.done, jnp.logical_not(task_done))
    return {
        "progress/episode_step_fraction": safe_jax_to_python(
            jnp.mean(timestep.state.env_steps / max_steps)
        ),
        "progress/task_completion_rate": safe_jax_to_python(jnp.mean(task_done)),
        "progress/timeout_rate": safe_jax_to_python(jnp.mean(timeout)),
    }
...
class MixedAgentTrainConfig:
    ...
    macro_conditional_entropy_scale: float = 0.05
    ...
    macro_move_cost_per_tile: float | None = None
...
def create_mixed_agent_env_config(
    ...
    Args:
        ...
        macro_move_cost_per_tile: Penalty per tile of macro movement path length
    ...
    if macro_move_cost_per_tile is not None:
        env_config = env_config._replace(macro_move_cost_per_tile=macro_move_cost_per_tile)
...
def make_mixed_agent_states(config: MixedAgentTrainConfig, env_params: EnvConfig = None, env_params_override: EnvConfig = None):
    ...
    if env_params is None:
        if env_params_override is not None:
            ...
            env_params = create_mixed_agent_env_config(
                ...
                macro_move_cost_per_tile=config.macro_move_cost_per_tile,
            ...
            if any([config.dump_bonus_mult, config.excavator_relocate_dumped_mult,
                    ...
                    config.macro_move_cost_per_tile]):
                ...
                if config.macro_move_cost_per_tile is not None:
                    print(f"   macro_move_cost_per_tile: {config.macro_move_cost_per_tile}")
    ...
    _print_macro_policy_debug(config, env, network_params)
    ...
    def make_mixed_agent_train(env, env_params, config):
        def train(rng: jax.Array, train_state: TrainState):
            ...
            def _update_step(runner_state, ent_coef_current, update_idx):
                ...
                meters_moved_sum = jnp.sum(transitions.meters_moved)
                macro_move_count_sum = jnp.sum(transitions.macro_move_count)
                env_step_count = transitions.meters_moved.size
                loss_info["train/avg_meters_moved_per_env_step"] = (
                    meters_moved_sum / jnp.maximum(env_step_count, 1)
                )
                loss_info["train/avg_meters_moved_per_macro_move"] = (
                    meters_moved_sum / jnp.maximum(macro_move_count_sum, 1.0)
                )
            ...
            for i in tqdm(range(config.num_updates), desc="Training"):
                ...
                if i % config.log_train_interval == 0:
                    ...
                    try:
                        ...
                        log_dict.update(episode_progress_metrics(timestep))
                        log_dict.update(digging_progress_metrics(timestep))
                    except Exception:
                        pass
                ...
                if i % config.log_eval_interval == 0:
                    ...
                    eval_stats = eval_stats_per_device._replace(
                        ...
                        action_8=eval_stats_per_device.action_8.sum(),
                        action_9=eval_stats_per_device.action_9.sum(),
                        action_10=eval_stats_per_device.action_10.sum(),
                    )
                    ...
                    loss_info_single.update(
                        {
                            ...
                            "eval/MOVE_TO_POSE %": eval_stats.action_8 / n,
                            "eval/SET_CABIN_ANGLE %": eval_stats.action_9 / n,
                            "eval/SET_BASE_ANGLE %": eval_stats.action_10 / n,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--macro_conditional_entropy_scale", type=float, default=0.05,
        help="Scale conditional macro-head entropy so MOVE_TO_POSE's 4096-way head does not dominate exploration."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--macro_move_cost_per_tile", type=float, default=None,
        help="Penalty per tile of macro MOVE_TO_POSE path length (overrides config preset)"
    )
    ...
    if args.config is not None:
        try:
            ...
            macro_move_cost_per_tile = preset.reward_multipliers.macro_move_cost_per_tile
    ...
    if args.macro_move_cost_per_tile is not None:
        macro_move_cost_per_tile = args.macro_move_cost_per_tile
    ...
    config = MixedAgentTrainConfig(
        ...
        macro_move_cost_per_tile=macro_move_cost_per_tile,
        ...
        macro_conditional_entropy_scale=args.macro_conditional_entropy_scale,
    )
