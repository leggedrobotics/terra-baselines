...
class MapsNet(nn.Module):
    ...
    def __call__(self, obs: dict[str, Array]):
        ...
        interaction_mask_1 = obs[20]
        interaction_mask_2 = obs[21]
        ...
        x = jnp.concatenate(
            (
                ...
                interaction_mask_1[..., None],
                interaction_mask_2[..., None],
            ),
            axis=-1,
        )
        x = self.cnn(x)
...
class SimplifiedCoupledCategoricalNet(nn.Module):
    """
    The full net for centralized dual-agent policy with shared parameters.
    """
    ...
    hidden_dim_pi: Sequence[int] = (128, 64, 32)
    ...
    def setup(self) -> None:
        ...
        self.mlp_pi = MLP(
            hidden_dim_layers=self.hidden_dim_pi + (num_actions * 2,),
            use_layer_norm=self.mlp_use_layernorm,
            last_layer_init_scaling=0.01,
        )
    ...
    def __call__(self, obs: Array) -> Array:
        # Process features for both agents using shared networks
        x_agent_state_1 = self.agent_state_net(obs[0])
        x_local_map_1 = self.local_map_net(obs[1:7])
        x_actions_1 = self.actions_net(obs[11])

        x_agent_state_2 = self.agent_state_net(obs[12])
        x_local_map_2 = self.local_map_net(obs[13:19])
        x_actions_2 = self.actions_net(obs[19])

        # Process shared map features
