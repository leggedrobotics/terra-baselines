...
def create_sub_task_action_map_64x64(action_map_data: jnp.ndarray,
                                    region_coords: tuple[int, int, int, int]) -> jnp.ndarray:
    """
    Creates a 64x64 action map for a sub-task from a 64x64 input, preserving only actions 
    that occurred inside the specified region. Outside the region, all values are reset to 0 (free).

    Args:
        action_map_data: Action map of size 64x64 (-1: dug, 0: free, >0: dumped).
        region_coords: (y_start, x_start, y_end, x_end), inclusive.

    Returns:
        A new 64x64 map with only the region's actions preserved at their original positions, 
        all else is 0.
    """
    y_start, x_start, y_end, x_end = region_coords
...
def create_sub_task_padding_mask_64x64(padding_mask_data: jnp.ndarray,
                                      region_coords: tuple[int, int, int, int]) -> jnp.ndarray:
    """
    Creates a 64x64 padding mask for a sub-task from a 64x64 input.

    Inside the region: preserves original traversability (0 or 1).
    Outside the region: sets everything to 1 (non-traversable).

    Args:
        padding_mask_data: Mask of size 64x64 (0: traversable, 1: non-traversable).
        region_coords: (y_start, x_start, y_end, x_end), inclusive.

    Returns:
        A 64x64 mask with only the region preserved at its original position; the rest is 1.
    """
    y_start, x_start, y_end, x_end = region_coords
...
def create_sub_task_traversability_mask_64x64(traversability_mask_data: jnp.ndarray,
                                             region_coords: tuple[int, int, int, int]) -> jnp.ndarray:
    """
    Creates a 64x64 traversability mask for a sub-task from a 64x64 input.

    Inside the region: preserves original values (-1: agent, 0: traversable, 1: non-traversable).
    Outside the region: sets everything to 1 (non-traversable).

    Args:
        traversability_mask_data: Mask of size 64x64 
                                  (-1: agent, 0: traversable, 1: non-traversable).
        region_coords: (y_start, x_start, y_end, x_end), inclusive.
...
def create_sub_task_dumpability_mask_64x64(dumpability_mask_data: jnp.ndarray,
                                          region_coords: tuple[int, int, int, int]) -> jnp.ndarray:
    """
    Creates a 64×64 dumpability mask for a sub-task from a 64x64 input.

    Inside the region: preserves original values (1: can dump, 0: can't dump).
    Outside the region: sets everything to 0 (can't dump).

    Args:
        dumpability_mask_data: Mask of size 64x64 (1: can dump, 0: can't).
        region_coords: (y_start, x_start, y_end, x_end), inclusive.

    Returns:
        A 64×64 mask with only the region preserved at its original position; the rest is 0.
    """
    y_start, x_start, y_end, x_end = region_coords
...
def save_mask(mask, mask_name, output_path, partition_index, map_step=None):

    os.makedirs(output_path, exist_ok=True)
    output_path = f"{output_path}/{mask_name}_partition_{partition_index}_step_{map_step}.png"

    h, w = mask.shape
    rgb_image = np.zeros((h, w, 3), dtype=np.uint8)

    rgb_image[mask == -1] = [0, 0, 255]     # Red for agent occupancy
    rgb_image[mask == 0]  = [255, 255, 255] # White for traversable
    rgb_image[mask == 1]  = [0, 0, 0]       # Black for non-traversable

    cv2.imwrite(output_path, rgb_image)
    print(f"{mask_name} saved to {output_path}")

def extract_target_map_region_64x64(global_target_map_data: jnp.ndarray,
                                    region_coords: tuple[int, int, int, int]) -> jnp.ndarray:
    """
    Extracts a 64x64 region from the global target map without modification.
    ...
    Returns:
        A 64x64 region extracted directly from the global map with original values preserved.
    """
    y_start, x_start, y_end, x_end = region_coords
    
    # Extract exactly 64x64 region from the global map starting at region coordinates
    return global_target_map_data[y_start:y_start+64, x_start:x_start+64]

def extract_action_map_region_64x64(action_map_data: jnp.ndarray,
                                   region_coords: tuple[int, int, int, int]) -> jnp.ndarray:
    """
    Extracts a 64x64 region from the action map without modification.
    ...
    Returns:
        A 64x64 region extracted directly from the action map with original values preserved.
    """
    y_start, x_start, y_end, x_end = region_coords

    # Extract exactly 64x64 region from the action map starting at region coordinates
    return action_map_data[y_start:y_start+64, x_start:x_start+64]

def extract_padding_mask_region_64x64(padding_mask_data: jnp.ndarray,
                                     region_coords: tuple[int, int, int, int]) -> jnp.ndarray:
    """
    Extracts a 64x64 region from the padding mask without modification.
    ...
    Returns:
        A 64x64 region extracted directly from the padding mask with original values preserved.
    """
    y_start, x_start, y_end, x_end = region_coords

    # Extract exactly 64x64 region from the padding mask starting at region coordinates
    return padding_mask_data[y_start:y_start+64, x_start:x_start+64]

def extract_traversability_mask_region_64x64(traversability_mask_data: jnp.ndarray,
                                            region_coords: tuple[int, int, int, int]) -> jnp.ndarray:
    """
    Extracts a 64x64 region from the traversability mask without modification.
    ...
    Returns:
        A 64x64 region extracted directly from the traversability mask with original values preserved.
    """
    y_start, x_start, y_end, x_end = region_coords

    # Extract exactly 64x64 region from the traversability mask starting at region coordinates
    return traversability_mask_data[y_start:y_start+64, x_start:x_start+64]

def extract_dumpability_mask_region_64x64(dumpability_mask_data: jnp.ndarray,
                                         region_coords: tuple[int, int, int, int]) -> jnp.ndarray:
    """
    Extracts a 64x64 region from the dumpability mask without modification.
    ...
    Returns:
        A 64x64 region extracted directly from the dumpability mask with original values preserved.
    """
    y_start, x_start, y_end, x_end = region_coords

    # Extract exactly 64x64 region from the dumpability mask starting at region coordinates
    return dumpability_mask_data[y_start:y_start+64, x_start:x_start+64]