...
def run_single_experiment(experiment_args):
    ...
    """
    # Suppress all prints except final stats
    with open(os.devnull, 'w') as devnull:
        with redirect_stdout(devnull):
            (model_name, model_key, num_timesteps, seed, run_name, 
             experiment_number, agent_checkpoint_path, level_index) = experiment_args
            
            # Import necessary modules inside the function to avoid pickling issues
            from utils.helpers import load_pkl_object
            from llm.env_manager_llm import EnvironmentsManager
            from llm.eval_llm import compute_stats_llm
            
            # Set level index if provided
            if level_index is not None:
                #import os
                os.environ['TERRA_LEVEL_INDEX'] = str(level_index)
            ...
            # Initialize environment manager for this process
    ...
    # This print will still show (outside the context manager)
    ...
    results = {
        ...
        'dig_tiles_per_target_map_init': info["dig_tiles_per_target_map_init"].item(),
        'dug_tiles_per_action_map': info["dug_tiles_per_action_map"].item()
    }
...
#     # Load model configuration
#     print(f"Experiment {experiment_number}: Loading RL agent configuration from: {agent_checkpoint_path}")
#     log = load_pkl_object(agent_checkpoint_path)
#     config = log["train_config"]
#     model_params = log["model"]
...
#     print(f"Experiment {experiment_number}: Running with seed {seed}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum number of parallel workers. If None, uses all available CPU cores."
    )
    ...
    # Prepare experiment arguments for each experiment
    ...
    for i in range(NUM_ENVS):
        experiment_args = (
            ...
            args.run_name,
            i + 1,  # experiment_number
            args.run_name,  # agent_checkpoint_path
            args.level_index
        )
        experiment_args_list.append(experiment_args)
    
    # Determine number of workers
    max_workers = args.max_workers or min(NUM_ENVS, mp.cpu_count())
    
    print(f"Running {NUM_ENVS} experiments in parallel with {max_workers} workers")
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        future_to_experiment = {
            executor.submit(run_single_experiment, exp_args): exp_args[5]  # exp_args[5] is experiment_number
            for exp_args in experiment_args_list
        }
        
        # Collect results as they complete
        completed_experiments = 0
        for future in as_completed(future_to_experiment):
            experiment_number = future_to_experiment[future]
            try:
                ...
                print(f"✅ Experiment {experiment_number} completed ({completed_experiments}/{NUM_ENVS})")
                
            except Exception as exc:
                print(f"❌ Experiment {experiment_number} generated an exception: {exc}")
    
    print(f"\nAll {NUM_ENVS} experiments completed!")
