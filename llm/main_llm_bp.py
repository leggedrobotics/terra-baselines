...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of different maps to process",
    )
    parser.add_argument(
    ...
    # Load model configuration once - THIS IS CRITICAL
    ...
    # Lists to store the best results from each map
    ...
    # Process each map
    for map_idx in range(args.n_maps):
        print(f"\n{'='*80}")
        print(f"PROCESSING MAP {map_idx + 1}/{args.n_maps}")
        print(f"{'='*80}")
        ...
        for partition_trial in range(args.n_partitions_per_map):
            ...
            try:
                ...
                # The key insight: each map_idx should get a significantly different seed
                # so that the environment generates different maps
                #trial_seed = base_seed + map_idx * 50000 + partition_trial * 100
                trial_seed = base_seed + map_idx * 50000 
        ...
        # Find the partition with highest coverage for this map
        if map_results:
            # Sort by coverage and get the best one
            ...
            # Store the best results from this map
            best_episode_done_once_list.append(best_result['episode_done_once'])
            best_episode_length_list.append(best_result['episode_length'])
            best_move_cumsum_list.append(best_result['move_cumsum'])
            best_do_cumsum_list.append(best_result['do_cumsum'])
            best_areas_list.append(best_result['areas'])
            best_dig_tiles_per_target_map_init_list.append(best_result['dig_tiles_per_target_map_init'])
            best_dug_tiles_per_action_map_list.append(best_result['dug_tiles_per_action_map'])
    ...
    # Calculate final statistics from best partitions only
    print(f"\n{'='*80}")
    print(f"FINAL RESULTS FROM BEST PARTITIONS")
    print(f"{'='*80}")
    print(f"Processed {len(best_episode_done_once_list)} maps")
    print(f"Each map tested with {args.n_partitions_per_map} random partitions")
    
    if best_episode_done_once_list and COMPUTE_BENCH_STATS:
        compute_stats_llm(
            best_episode_done_once_list, 
            best_episode_length_list, 
            best_move_cumsum_list,
        ...
        # Additional statistics for best partitions
        coverages = [dug/total for dug, total in zip(best_dug_tiles_per_action_map_list, best_dig_tiles_per_target_map_init_list) if total > 0]
        if coverages:
            ...
            results_data = {
                ...
                'best_episode_done_once': best_episode_done_once_list,
                'best_episode_length': best_episode_length_list,
                'best_move_cumsum': best_move_cumsum_list,
                'best_do_cumsum': best_do_cumsum_list,
                'best_areas': best_areas_list,
                'best_dig_tiles_per_target_map_init': best_dig_tiles_per_target_map_init_list,
                'best_dug_tiles_per_action_map': best_dug_tiles_per_action_map_list,
                'experiment_config': {
                    'model_name': args.model_name,
                    'n_maps': args.n_maps,
                    'n_partitions_per_map': args.n_partitions_per_map,
            ...
            results_filename = f"best_partition_results_{args.model_name.replace('/', '_')}_{timestamp}.json"
