...
if __name__ == "__main__":
    ...
    print(f"\nRunning map {args.map_idx}/{args.n_maps} with {args.n_partitions_per_map} partition trials")
    ...
    if result:
        print(f"\n{'='*80}")
        print(f"MAP {args.map_idx} COMPLETED SUCCESSFULLY")
        print(f"Best coverage: {result['coverage']:.4f}")
        print(f"{'='*80}")
    else:
        print(f"\n{'='*80}")
        print(f"MAP {args.map_idx} FAILED")
        print(f"{'='*80}")
        exit(1)
