...
| Generalist compiler throughput | **Repair implemented; Euler gate pending** | Recovery disabled XLA autotuning and regressed to 3,124.6 steps/s versus 15,800--16,771 in matched 4-GPU controls. The replacement keeps level-4 profiling, disables the failing cuDNN frontend runner, pins deterministic algorithm selection, pins native u3,500 checkpoint SHA `f84a6cdf...`, and adds exact-shape analytic gradient checks. | Run a fast one-GPU Euler compiler canary, then require five finite four-GPU resumed updates and post-compile median at least 12,000 steps/s before starting replacement production or cancelling jobs `11626135`/`11626137`. |
...
unchanged by the deterministic training recovery described below. It is a new matched u14/u39/u40
