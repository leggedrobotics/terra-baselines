...
| Relay and cleanup underexposure | **Implemented/training; fixed gate pending** | The 37-condition `trench_align_generalist_partial_v1` recipe integrates the bounded u0--u10k partial-start schedule, reset provenance, full-start-only mastery updates, strict trench alignment, and bank-bound native resume. Job `11626135` has finite checkpoints beyond u3,500; this establishes training only, not relay benefit. | Repair the compiler throughput regression, then judge only full-start fixed panels plus recurrence/relay strata. Do not credit partial starts as evaluation successes. |
| Generalist compiler throughput | **Revised repair implemented; Euler gate pending** | Recovery disabled XLA autotuning and regressed to 3,124.6 steps/s versus 15,800--16,771 in matched controls. Frontend-off deterministic selection was rejected at 591.69/579.32 steady steps/s on an exclusive RTX 3060, and its pending Euler chain was cancelled before allocation. The revised candidate restores the default frontend, excludes known-bad nodes, pins native u3,500 SHA `f84a6cdf...`, and analytically checks the failing shapes. | Run a fast one-GPU Euler compiler canary, then require five finite four-GPU resumed updates and post-compile median at least 12,000 steps/s before starting replacement production or cancelling jobs `11626135`/`11626137`. |
...
| Actor GRU | **Trained/evaluated; residual traps remain** | The concat-skip GRU64 reaches 674/720 on the fixed development panel at u44k versus 652/720 for feed-forward u44k and 670/720 for feed-forward u86k. Zeroing carry changes 17 actions across all 12 recurrence targets and raises target no-effect rate from 24.3% to 50.9%, proving memory use; both carry arms still solve only 9/12. | Keep the complete recurrent treatment as the sample-efficient capability leader, but do not call it a recurrence-only ablation. Target slots 250, 247, and 177 with the feedback/failure panels before changing architecture again. |
...
`--xla_gpu_enable_cudnn_frontend=false`.  This separate inference contract is
unchanged by the training-runtime recovery described below. It is a new matched
u14/u39/u40 contract rather than a bit-identical replay of the old backend, but the u14
