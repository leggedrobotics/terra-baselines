...
| Actor GRU | **Training; update-1 gate pending** | Stall age delays some traps but saturates, and material-changing ping-pong resets it. The actor-only GRU-64 capability run is Slurm job `10949464` on 4 RTX 4090 GPUs; its exact contract is in [`V8_RECURRENT_ACTOR_GRU_20260817.md`](V8_RECURRENT_ACTOR_GRU_20260817.md). | Require the first finite PPO update, then judge only checkpointed policies on the fixed full-start and recurrence panels. A stateless-GRU/sequence-batched feed-forward control was deliberately omitted, so interpret gains practically rather than as component attribution. |
