...
| Actor GRU | **Implemented; 4-GPU launch pending** | Stall age delays some traps but saturates, and material-changing ping-pong resets it. The actor-only GRU-64 implementation and exact experiment contract are in [`V8_RECURRENT_ACTOR_GRU_20260817.md`](V8_RECURRENT_ACTOR_GRU_20260817.md). | Run the authorized fresh 4-GPU capability experiment. A stateless-GRU/sequence-batched feed-forward control is deliberately omitted, so interpret gains practically rather than as component attribution. |
